...
public class MyMazeBug extends MazeBug {
    ...
    private int[] factor = { TEN, TEN, TEN, TEN };
    ...
    private void getNext(ArrayList<Location> validList) {
        Location[] validloc = { null, null, null, null };
        // get the calidlist location
        for (int i = 0; i < validList.size(); i++) {
            if (validList.get(i).getDirectionToward(getLocation()) == Location.SOUTH) {
                // north
                validloc[0] = validList.get(i);
            }
            if (validList.get(i).getDirectionToward(getLocation()) == Location.NORTH) {
                // south
                validloc[1] = validList.get(i);
            }
            if (validList.get(i).getDirectionToward(getLocation()) == Location.WEST) {
                // east
                validloc[3] = validList.get(i);
            }
            if (validList.get(i).getDirectionToward(getLocation()) == Location.EAST) {
                // west
                validloc[2] = validList.get(i);
            }
        }
